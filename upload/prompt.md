website ini ada 2 user: Admin and User. User adalah orang yang melihat web ini dengan tujuan untuk memahami kemampuanku, biasanya untuk HR. Admin disini adalah aku sebagai orang yang memperkenalkan diri kepada recruiter, khususkan page untuk admin supaya admin bisa melakukan edit di seluruh element yang ada di website, tujuannya agar mempermudah edit dan update portfolio sang admin, tentu saja dilengkapi dengan auth.

Admin Page:
1. Autentikasi & Keamanan (Auth)
Secure Login Page: Halaman login khusus dengan proteksi (misalnya menggunakan email/password atau OAuth).

Session Management: Fitur auto-logout jika idle terlalu lama, dan perlindungan rute (route protection) agar halaman admin tidak bisa diakses tanpa token/sesi yang valid.

2. Manajemen "Introduction" (Hero Section)
Profile Editor: Form untuk mengedit Nama, Headline (misal: "Data Analyst & Full-stack Developer"), dan Bio/Summary.

Media Uploader: Tombol untuk mengganti foto profil atau background/cover.

Resume/CV Manager: Tombol untuk mengunggah file PDF CV terbaru, sehingga tombol "Download CV" di halaman depan selalu menyajikan versi paling mutakhir.

3. Manajemen "Skills"
CRUD (Create, Read, Update, Delete) Skill: Form untuk menambah skill baru.

Kategorisasi: Dropdown untuk mengelompokkan skill ke dalam Soft Skills, Hard Skills, atau Language/Programming Language.

Icon Manager: Input untuk memasukkan nama/kode icon (jika menggunakan library seperti React Icons/Lucide) atau fitur unggah file .svg/.png untuk logo brand.

4. Manajemen "Experience"
Form Detail Pengalaman: Input untuk Posisi/Role, Nama Perusahaan, dan Tanggal (Bulan & Tahun Mulai - Selesai, serta opsi checkbox "Present" jika masih bekerja di sana).

Rich Text Editor: Editor teks (seperti tampilan Microsoft Word sederhana) untuk menulis deskripsi pekerjaan secara rapi menggunakan bullet points, bold, atau italic.

Media & Tautan: Kolom input URL untuk menyematkan (embed) video atau mengunggah gambar overview. Terdapat juga kolom untuk memasukkan tautan eksternal ke publikasi atau proyek terkait.

5. Manajemen "Projects"
Kategorisasi Status: Toggle atau dropdown untuk menetapkan project sebagai 'Past Project' atau 'In Development'.

Project Details: Input judul proyek dan Rich Text Editor untuk deskripsi.

Relasi Project & Skills: Fitur multi-select atau checkbox yang menarik data langsung dari tabel "Skills". Jadi, kamu tinggal mencentang skill apa saja (misal: Next.js, Python, Tailwind) yang digunakan dalam project tersebut, dan otomatis akan muncul sebagai badge atau ikon di card proyek.

Media Gallery: Fitur untuk mengunggah thumbnail (untuk grid 2x2) dan media tambahan (gambar/video) jika proyek di-klik oleh HRD untuk detail lebih lanjut.

6. Manajemen "Contact" & Pesan
Social Links Editor: Form sederhana untuk memperbarui URL GitHub, LinkedIn, dan nomor WhatsApp.

Inbox Dashboard (Opsional tapi disarankan): Tabel untuk melihat, membaca, dan menghapus pesan yang dikirim oleh pengunjung melalui form Contact. Ini sangat berguna sebagai backup jika email forwarding dari web gagal terkirim.

7. Fitur Pendukung Admin (Quality of Life)
Draft / Publish Status: Toggle switch (Aktif/Nonaktif) pada setiap Experience dan Project. Ini memungkinkanmu menyimpan draft proyek yang sedang kamu tulis tanpa harus menampilkannya ke publik sebelum benar-benar siap.

Drag-and-Drop Reordering: Kemampuan untuk menggeser urutan Skills atau Projects secara manual di dashboard admin, sehingga kamu bisa menonjolkan proyek paling relevan di urutan teratas tanpa terikat tanggal pembuatan.

content yang harus ada di web-ku:
1. Introduction dan dilengkapi toombol Download CV
2. Skills yang dikelompokkan menjadi (Soft Skills, Hard Skill, Language/programming language) dilengkapi dengan icon brandnya.
3. Pengalaman, yang di lengkapi deskripsi, gambar/video overview, link to project.
4. project, ada dua kategori: 'Past Project' dan 'In Development'. Berbentuk card dengan grid 2x2, dilengkapi deskripsi, gambar/video/embed, serta ada hubungan dengan soft skill dan hard skill.
5. My Taste. Berisi embed Apple Music yang secara manual bisa di putar oleh user, ada juga Currently Brewing, Fitness Workout
6. Contact Me. buat fitur form untuk user mengirim email, dan tampilkan link github, linkedin, whatsapp

tech stack:
npm v10.9.4, react, supabase (if needed), vercel, typescript, next.js, github, 

additional features:
1. Spotlight Card for Project section
   ```
   npx shadcn@latest add @kokonutui/spotlight-cards
   ```
2. Smooth Tab for Switch User
   ```
...
```
4. 3D Wrapper for form at Contact Me section
   ```
   npx shadcn@latest add https://badtz-ui.com/r/3d-wrapper.json
   ```
5. Text Effect only use on Introduction Description exclude Name
   ```
   npx shadcn@latest add https://badtz-ui.com/r/blur-reveal.json
   ```
6. Expandable Card for Project Section
   ```
   npx shadcn@latest add https://badtz-ui.com/r/expandable-card.json
   ```
7. Shine Button use for "Download CV"
   ```
   npx lightswind@latest add shine-button
   ```
8. Drag Order List for Admin Manage the Project section and Experience Section
   ```
   npx lightswind@latest add drag-order-list
   ```
9. Scroll Timeline for Experience Career 
   ```
   npx lightswind@latest add scroll-timeline
   ```
10. Bento Grid for My Taste section
    ```
